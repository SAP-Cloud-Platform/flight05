CLASS /dmo/cl_flight_legacy05 DEFINITION
  PUBLIC
  FINAL
  CREATE PRIVATE
  GLOBAL FRIENDS /dmo/cl_flight_data_generat_05.

  PUBLIC SECTION.
    INTERFACES /dmo/if_flight_legacy05.
    ...
    CLASS-METHODS: get_instance RETURNING VALUE(ro_instance) TYPE REF TO /dmo/cl_flight_legacy05.
    ...
    METHODS create_travel IMPORTING is_travel             TYPE /dmo/if_flight_legacy05=>ts_travel_in
                                    it_booking            TYPE /dmo/if_flight_legacy05=>tt_booking_in OPTIONAL
                                    it_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_in OPTIONAL
                          EXPORTING es_travel             TYPE /dmo/travel05
                                    et_booking            TYPE /dmo/if_flight_legacy05=>tt_booking
                                    et_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                                    et_messages           TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS update_travel IMPORTING is_travel              TYPE /dmo/if_flight_legacy05=>ts_travel_in
                                    is_travelx             TYPE /dmo/if_flight_legacy05=>ts_travel_inx
                                    it_booking             TYPE /dmo/if_flight_legacy05=>tt_booking_in OPTIONAL
                                    it_bookingx            TYPE /dmo/if_flight_legacy05=>tt_booking_inx OPTIONAL
                                    it_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_in OPTIONAL
                                    it_booking_supplementx TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_inx OPTIONAL
                          EXPORTING es_travel              TYPE /dmo/travel05
                                    et_booking             TYPE /dmo/if_flight_legacy05=>tt_booking
                                    et_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                                    et_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS delete_travel IMPORTING iv_travel_id TYPE /dmo/travel_id05
                          EXPORTING et_messages  TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS get_travel IMPORTING iv_travel_id           TYPE /dmo/travel_id05
    ...
    METHODS save.
    METHODS initialize.
    METHODS convert_messages IMPORTING it_messages TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                             EXPORTING et_messages TYPE /dmo/if_flight_legacy05=>tt_message.
  PROTECTED SECTION.
  PRIVATE SECTION.
    CLASS-DATA go_instance TYPE REF TO /dmo/cl_flight_legacy05.
    ...
    CLASS-METHODS:
      ...
      calculate_flight_price
        IMPORTING
          iv_seats_occupied_percent TYPE /dmo/plane_seats_occupied05
          iv_flight_distance        TYPE i
        RETURNING
          VALUE(rv_price)           TYPE /dmo/flight_price05 ##RELAX.

    METHODS lock_travel IMPORTING iv_lock TYPE abap_bool
                        RAISING   /dmo/cx_flight_legacy05 ##RELAX ##NEEDED.

    METHODS _resolve_attribute IMPORTING iv_attrname      TYPE scx_attrname
                                         ix               TYPE REF TO /dmo/cx_flight_legacy05
    ...
    METHODS _determine EXPORTING et_messages           TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                       CHANGING  cs_travel             TYPE /dmo/travel05
                                 ct_booking            TYPE /dmo/if_flight_legacy05=>tt_booking
                                 ct_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement.
    METHODS _determine_travel_total_price CHANGING cs_travel             TYPE /dmo/travel05
                                                   ct_booking            TYPE /dmo/if_flight_legacy05=>tt_booking
                                                   ct_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                                                   ct_messages           TYPE /dmo/if_flight_legacy05=>tt_if_t100_message ##NEEDED.
    METHODS _convert_currency IMPORTING iv_currency_code_source TYPE /dmo/currency_code05
                                        iv_currency_code_target TYPE /dmo/currency_code05
                                        iv_amount               TYPE /dmo/total_price05
                              RETURNING VALUE(rv_amount)        TYPE /dmo/total_price05.
ENDCLASS.



CLASS /dmo/cl_flight_legacy05 IMPLEMENTATION.
  ...
  METHOD convert_messages.
    ...
    LOOP AT it_messages INTO DATA(lr_error) ##INTO_OK.
      ...
      IF lr_error IS INSTANCE OF /dmo/cx_flight_legacy05.
        DATA(lx) = CAST /dmo/cx_flight_legacy05( lr_error ).
  ...
  METHOD create_travel.
    ...
    lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( CORRESPONDING #( is_travel ) ) )
                                                            it_travelx  = VALUE #( ( travel_id = is_travel-travel_id  action_code = /dmo/if_flight_legacy05=>action_code-create ) )
    ...
    " Bookings
    IF et_messages IS INITIAL.
      DATA lt_booking  TYPE /dmo/if_flight_legacy05=>tt_booking.
      DATA lt_bookingx TYPE /dmo/if_flight_legacy05=>tt_bookingx.
      LOOP AT it_booking INTO DATA(ls_booking_in).
        DATA ls_booking TYPE /dmo/booking05.
        ls_booking = CORRESPONDING #( ls_booking_in ).
        ls_booking-travel_id = es_travel-travel_id.
        INSERT ls_booking INTO TABLE lt_booking.
        INSERT VALUE #( travel_id = ls_booking-travel_id  booking_id = ls_booking-booking_id  action_code = /dmo/if_flight_legacy05=>action_code-create ) INTO TABLE lt_bookingx.
      ENDLOOP.
    ...
    " Booking Supplements
    IF et_messages IS INITIAL.
      DATA lt_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement.
      DATA lt_booking_supplementx TYPE /dmo/if_flight_legacy05=>tt_booking_supplementx.
      LOOP AT it_booking_supplement INTO DATA(ls_booking_supplement_in).
        DATA ls_booking_supplement TYPE /dmo/book_sup_05.
        ...
        INSERT VALUE #( travel_id             = ls_booking_supplement-travel_id
                        ...
                        action_code           = /dmo/if_flight_legacy05=>action_code-create ) INTO TABLE lt_booking_supplementx.
      ENDLOOP.
      IF et_messages IS INITIAL.
  ...
  METHOD delete_travel.
    ...
    IF et_messages IS INITIAL.
      lcl_booking_supplement_buffer=>get_instance( )->cud_prep( EXPORTING it_booking_supplement  = CORRESPONDING #( lt_booking_supplement MAPPING travel_id             = travel_id
                                                                          ...
                                                                          it_booking_supplementx = VALUE #( FOR ls_bs IN lt_booking_supplement ( action_code           = /dmo/if_flight_legacy05=>action_code-delete
    ...
    IF et_messages IS INITIAL.
      lcl_booking_buffer=>get_instance( )->cud_prep( EXPORTING it_booking         = CORRESPONDING #( lt_booking MAPPING travel_id = travel_id  booking_id = booking_id EXCEPT * )
                                                               it_bookingx        = VALUE #( FOR ls_b IN lt_booking ( action_code = /dmo/if_flight_legacy05=>action_code-delete  travel_id = ls_b-travel_id  booking_id = ls_b-booking_id ) )
                                                               iv_no_delete_check = abap_true " No existence check required
    ...
    IF et_messages IS INITIAL.
      lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel          = VALUE #( ( travel_id = iv_travel_id ) )
                                                              it_travelx         = VALUE #( ( travel_id = iv_travel_id  action_code = /dmo/if_flight_legacy05=>action_code-delete ) )
                                                              iv_no_delete_check = abap_true " No existence check required
                                                    IMPORTING et_messages        = lt_messages ).
      APPEND LINES OF lt_messages TO et_messages.
    ENDIF.
...
*     CALL FUNCTION 'ENQUEUE_/DMO/ETRAVEL05'
*       EXCEPTIONS
*         foreign_lock   = 1
*         system_failure = 2
*         OTHERS         = 3.
*     IF sy-subrc <> 0.
*       RAISE EXCEPTION TYPE /dmo/cx_flight_legacy05
*         EXPORTING
*           textid = /dmo/cx_flight_legacy05=>travel_lock.
*     ENDIF.
*   ELSE.
*     CALL FUNCTION 'DEQUEUE_/DMO/ETRAVEL05'.
*   ENDIF.
  ENDMETHOD.
  ...
  METHOD update_travel.
    ...
    IF is_travel-travel_id IS INITIAL.
      ...
      RETURN.
    ENDIF.
    DATA ls_travelx TYPE /dmo/if_flight_legacy05=>ts_travelx.
    ls_travelx = CORRESPONDING #( is_travelx ).
    ls_travelx-action_code = /dmo/if_flight_legacy05=>action_code-update.
    lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( CORRESPONDING #( is_travel ) ) )
    ...
    IF et_messages IS INITIAL.
      ...
      LOOP AT it_bookingx INTO DATA(ls_booking_inx).
        DATA ls_bookingx TYPE /dmo/if_flight_legacy05=>ts_bookingx.
    ...
    IF et_messages IS INITIAL.
      ...
      IF et_messages IS INITIAL.
        LOOP AT it_booking_supplementx INTO DATA(ls_booking_supplement_inx).
          DATA ls_booking_supplementx TYPE /dmo/if_flight_legacy05=>ts_booking_supplementx.
    ...
    IF    et_messages IS INITIAL
      ...
      AND line_exists( lt_bookingx[ action_code = CONV /dmo/action_code05( /dmo/if_flight_legacy05=>action_code-delete ) ] ).
      ...
      LOOP AT lt_booking_supplement_del ASSIGNING FIELD-SYMBOL(<s_booking_supplement_del>).
        READ TABLE lt_bookingx TRANSPORTING NO FIELDS WITH KEY action_code = CONV /dmo/action_code05( /dmo/if_flight_legacy05=>action_code-delete )
      ...
      lcl_booking_supplement_buffer=>get_instance( )->cud_prep( EXPORTING it_booking_supplement  = CORRESPONDING #( lt_booking_supplement_del MAPPING travel_id             = travel_id
                                                                                                                                                      booking_id            = booking_id
                                                                                                                                                      booking_supplement_id = booking_supplement_id EXCEPT * )
                                                                          it_booking_supplementx = VALUE #( FOR ls_bs IN lt_booking_supplement_del ( action_code           = /dmo/if_flight_legacy05=>action_code-delete
                                                                                                                                                     travel_id             = ls_bs-travel_id
                                                                                                                                                     booking_id            = ls_bs-booking_id
                                                                                                                                                     booking_supplement_id = ls_bs-booking_supplement_id ) )
                                                                          iv_no_delete_check     = abap_true " No existence check required
      ...
      IF et_messages IS INITIAL.
        ...
        LOOP AT et_booking ASSIGNING FIELD-SYMBOL(<s_booking>).
          LOOP AT it_bookingx TRANSPORTING NO FIELDS WHERE booking_id = <s_booking>-booking_id
            AND ( action_code = CONV /dmo/action_code05( /dmo/if_flight_legacy05=>action_code-create ) OR action_code = CONV /dmo/action_code05( /dmo/if_flight_legacy05=>action_code-update ) ).
            EXIT.
          ENDLOOP.
          IF sy-subrc <> 0.
            DELETE et_booking.
  ...
  METHOD _determine_travel_total_price.
    DATA lv_add TYPE /dmo/total_price05.
    ...
    IF lv_currency_code_target IS NOT INITIAL.
      ...
      lcl_travel_buffer=>get_instance( )->cud_prep( EXPORTING it_travel   = VALUE #( ( travel_id = cs_travel-travel_id  total_price = cs_travel-total_price  currency_code = cs_travel-currency_code ) )
                                                              it_travelx  = VALUE #( ( action_code = /dmo/if_flight_legacy05=>action_code-update  travel_id = cs_travel-travel_id  total_price = abap_true  currency_code = abap_true ) )
                                                    IMPORTING et_messages = DATA(lt_messages) ).
