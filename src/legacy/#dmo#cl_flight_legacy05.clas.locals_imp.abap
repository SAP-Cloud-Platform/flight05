CLASS lcl_common_checks DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    CLASS-METHODS is_currency_code_valid IMPORTING iv_currency_code   TYPE /dmo/currency_code05
                                         CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                         RETURNING VALUE(rv_is_valid) TYPE abap_boolean.
    CLASS-METHODS is_customer_id_valid IMPORTING iv_customer_id     TYPE /dmo/customer_id05
                                       CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                       RETURNING VALUE(rv_is_valid) TYPE abap_boolean.
  PRIVATE SECTION.
    CLASS-DATA mt_currency_code TYPE SORTED TABLE OF /dmo/currency_code05 WITH UNIQUE KEY table_line.
    CLASS-DATA mt_customer_id TYPE SORTED TABLE OF /dmo/customer_id05 WITH UNIQUE KEY table_line.
ENDCLASS.
...
CLASS lcl_common_checks IMPLEMENTATION.
  METHOD is_currency_code_valid.
    ...
    ELSE.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>currency_unknown  currency_code = iv_currency_code ) TO ct_messages.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD is_customer_id_valid.
    ...
    IF mt_customer_id IS INITIAL.
      ...
      SELECT DISTINCT customer_id FROM /dmo/customer05 INTO TABLE @mt_customer_id.
    ENDIF.
    READ TABLE mt_customer_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = iv_customer_id.
    IF sy-subrc = 0.
      rv_is_valid = abap_true.
    ELSE.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>customer_unkown  customer_id = iv_customer_id ) TO ct_messages.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
...
CLASS lcl_booking_supplement_buffer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    METHODS cud_prep IMPORTING it_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                               it_booking_supplementx TYPE /dmo/if_flight_legacy05=>tt_booking_supplementx
                               iv_no_delete_check     TYPE abap_bool OPTIONAL
                     EXPORTING et_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                               et_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    ...
    METHODS get IMPORTING it_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
  ...
  PRIVATE SECTION.
    ...
    " Main buffer
    DATA: mt_create_buffer TYPE /dmo/if_flight_legacy05=>tt_booking_supplement,
          mt_update_buffer TYPE /dmo/if_flight_legacy05=>tt_booking_supplement,
          mt_delete_buffer TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking_supplement,
          mt_update_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking_supplement,
          mt_delete_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_key.

    DATA mt_supplement TYPE SORTED TABLE OF /dmo/suppleme_05 WITH UNIQUE KEY supplement_id.

    METHODS _create IMPORTING it_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                    EXPORTING et_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                              et_messages           TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _update IMPORTING it_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                              it_booking_supplementx TYPE /dmo/if_flight_legacy05=>tt_booking_supplementx
                    EXPORTING et_booking_supplement  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                              et_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _delete IMPORTING it_booking_supplement TYPE /dmo/if_flight_legacy05=>tt_booking_supplement
                              iv_no_delete_check    TYPE abap_bool
                    EXPORTING et_messages           TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.

    METHODS _check IMPORTING is_booking_supplement  TYPE /dmo/book_sup_05
                             is_booking_supplementx TYPE /dmo/if_flight_legacy05=>ts_booking_supplementx OPTIONAL
                             iv_change_mode         TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                   CHANGING  ct_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _check_supplement IMPORTING is_booking_supplement  TYPE /dmo/book_sup_05
                                        is_booking_supplementx TYPE /dmo/if_flight_legacy05=>ts_booking_supplementx OPTIONAL
                                        iv_change_mode         TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                              CHANGING  ct_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                              RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_booking_supplement  TYPE /dmo/book_sup_05
                                           is_booking_supplementx TYPE /dmo/if_flight_legacy05=>ts_booking_supplementx OPTIONAL
                                           iv_change_mode         TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                                 CHANGING  ct_messages            TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _determine IMPORTING iv_change_mode        TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                       CHANGING  cs_booking_supplement TYPE /dmo/book_sup_05.
ENDCLASS.
...
CLASS lcl_booking_supplement_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    TYPES: BEGIN OF ts_travel_booking_suppl_id,
             travel_id             TYPE /dmo/travel_id05,
             booking_id            TYPE /dmo/booking_id05,
             booking_supplement_id TYPE /dmo/booking_supplement_id05,
    ...
    SELECT FROM /dmo/book_sup_05 FIELDS travel_id, booking_id, booking_supplement_id
    ...
    LOOP AT it_booking_supplement INTO DATA(ls_booking_supplement_create) ##INTO_OK.
      ...
      IF ls_booking_supplement_create-booking_supplement_id IS INITIAL.
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>booking_supplement_no_key
      ...
      IF sy-subrc = 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>booking_supplement_exists
                                          travel_id  = ls_booking_supplement_create-travel_id
                                          booking_id = ls_booking_supplement_create-booking_id
      ...
      IF _check( EXPORTING is_booking_supplement = ls_booking_supplement_create
                           iv_change_mode        = /dmo/cl_flight_legacy05=>change_mode-create
                 CHANGING  ct_messages           = et_messages ) = abap_false.
        RETURN.
      ENDIF.

      " standard determinations
      _determine( EXPORTING iv_change_mode        = /dmo/cl_flight_legacy05=>change_mode-create
  ...
  METHOD _update.
    ...
    LOOP AT it_booking_supplement ASSIGNING FIELD-SYMBOL(<s_booking_supplement_update>) WHERE booking_supplement_id = 0.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_supplement_no_key  travel_id = <s_booking_supplement_update>-travel_id  booking_id = <s_booking_supplement_update>-booking_id ) TO et_messages.
      RETURN.
    ENDLOOP.

    DATA lt_book_suppl TYPE SORTED TABLE OF /dmo/book_sup_05 WITH UNIQUE KEY travel_id  booking_id  booking_supplement_id.
    SELECT * FROM /dmo/book_sup_05 FOR ALL ENTRIES IN @it_booking_supplement WHERE travel_id             = @it_booking_supplement-travel_id
    ...
    FIELD-SYMBOLS <s_buffer_booking_supplement> TYPE /dmo/book_sup_05.
    DATA ls_buffer_booking_supplement TYPE /dmo/book_sup_05.
    LOOP AT it_booking_supplement ASSIGNING <s_booking_supplement_update>.
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW /dmo/cx_flight_legacy05( textid                = /dmo/cx_flight_legacy05=>booking_supplement_unknown
      ...
      IF sy-subrc <> 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid                = /dmo/cx_flight_legacy05=>booking_supplement_no_control
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF <s_booking_supplementx>-supplement_id = abap_true AND <s_booking_supplement_update>-supplement_id <> <s_buffer_booking_supplement>-supplement_id.
        ...
        APPEND NEW /dmo/cx_flight_legacy05( textid                = /dmo/cx_flight_legacy05=>booking_supplement_suppl_id_u
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF   ( <s_booking_supplementx>-price = abap_true  AND <s_booking_supplementx>-currency_code = abap_false )
        ...
        APPEND NEW /dmo/cx_flight_legacy05( textid                = /dmo/cx_flight_legacy05=>booking_supplement_pri_curr_u
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF _check( EXPORTING is_booking_supplement  = <s_buffer_booking_supplement>
                           ...
                           iv_change_mode         = /dmo/cl_flight_legacy05=>change_mode-update
                 CHANGING  ct_messages            = et_messages ) = abap_false.
        RETURN.
      ENDIF.
      ...
      _determine( EXPORTING iv_change_mode        = /dmo/cl_flight_legacy05=>change_mode-update
  ...
  METHOD _delete.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_book_suppl_db TYPE /dmo/if_flight_legacy05=>tt_booking_supplement_key.
      SELECT travel_id, booking_id, booking_supplement_id FROM /dmo/book_sup_05 FOR ALL ENTRIES IN @lt_booking_supplement
  ...
  METHOD save.
    ...
    INSERT /dmo/book_sup_05 FROM TABLE @mt_create_buffer.
    UPDATE /dmo/book_sup_05 FROM TABLE @mt_update_buffer.
    DELETE /dmo/book_sup_05 FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD cud_prep.
    ...
    DATA lt_booking_supplement_c  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement.
    DATA lt_booking_supplement_u  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement.
    DATA lt_booking_supplement_d  TYPE /dmo/if_flight_legacy05=>tt_booking_supplement.
    DATA lt_booking_supplementx_u TYPE /dmo/if_flight_legacy05=>tt_booking_supplementx.
    LOOP AT it_booking_supplement ASSIGNING FIELD-SYMBOL(<s_booking_supplement>).
      ...
      IF sy-subrc <> 0.
        ...
        RETURN.
      ENDIF.
      CASE CONV /dmo/if_flight_legacy05=>action_code_enum( <s_booking_supplementx>-action_code ).
        WHEN /dmo/if_flight_legacy05=>action_code-create.
          INSERT <s_booking_supplement>  INTO TABLE lt_booking_supplement_c.
        WHEN /dmo/if_flight_legacy05=>action_code-update.
        ...
        WHEN /dmo/if_flight_legacy05=>action_code-delete.
  ...
  METHOD _check_supplement.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_booking_supplementx-supplement_id = abap_true ).
      IF mt_supplement IS INITIAL.
        SELECT * FROM /dmo/suppleme_05 INTO TABLE @mt_supplement.
      ENDIF.
      READ TABLE mt_supplement TRANSPORTING NO FIELDS WITH TABLE KEY supplement_id = is_booking_supplement-supplement_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>supplement_unknown  supplement_id = is_booking_supplement-supplement_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create AND is_booking_supplement-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_booking_supplementx-currency_code = abap_true ).
  ...
  METHOD _determine.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create.
...
CLASS lcl_booking_buffer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    METHODS save.
    METHODS initialize.
    METHODS check_booking_id IMPORTING iv_travel_id       TYPE /dmo/travel_id05
    ...
    "! @parameter iv_no_delete_check | In some cases we do not need to check the existence of a record to be deleted, as this check has been done before.
    "!                               | E.g. delete all subnodes of a node to be deleted.  In this case we have read the subnodes to get their keys.
    METHODS cud_prep IMPORTING it_booking         TYPE /dmo/if_flight_legacy05=>tt_booking
                               it_bookingx        TYPE /dmo/if_flight_legacy05=>tt_bookingx
                               iv_no_delete_check TYPE abap_bool OPTIONAL
                     EXPORTING et_booking         TYPE /dmo/if_flight_legacy05=>tt_booking
                               et_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    "! Add content of the temporary buffer to the real buffer and clear the temporary buffer
    METHODS cud_copy.
    "! Discard content of the temporary buffer
    ...
    METHODS get IMPORTING it_booking             TYPE /dmo/if_flight_legacy05=>tt_booking
                          iv_include_buffer      TYPE abap_boolean
                          iv_include_temp_buffer TYPE abap_boolean
                EXPORTING et_booking             TYPE /dmo/if_flight_legacy05=>tt_booking.
  ...
  PRIVATE SECTION.
    ...
    " Main buffer
    DATA: mt_create_buffer TYPE /dmo/if_flight_legacy05=>tt_booking,
          mt_update_buffer TYPE /dmo/if_flight_legacy05=>tt_booking,
          mt_delete_buffer TYPE /dmo/if_flight_legacy05=>tt_booking_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking,
          mt_update_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking,
          mt_delete_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_booking_key.
    ...
    METHODS _create IMPORTING it_booking  TYPE /dmo/if_flight_legacy05=>tt_booking
                    EXPORTING et_booking  TYPE /dmo/if_flight_legacy05=>tt_booking
                              et_messages TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _update IMPORTING it_booking  TYPE /dmo/if_flight_legacy05=>tt_booking
                              it_bookingx TYPE /dmo/if_flight_legacy05=>tt_bookingx
                    EXPORTING et_booking  TYPE /dmo/if_flight_legacy05=>tt_booking
                              et_messages TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _delete IMPORTING it_booking         TYPE /dmo/if_flight_legacy05=>tt_booking
                              iv_no_delete_check TYPE abap_bool
                    EXPORTING et_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.

    METHODS _check IMPORTING is_booking         TYPE /dmo/booking05
                             is_bookingx        TYPE /dmo/if_flight_legacy05=>ts_bookingx OPTIONAL
                             iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                   CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_booking_date IMPORTING is_booking         TYPE /dmo/booking05
                                          is_bookingx        TYPE /dmo/if_flight_legacy05=>ts_bookingx OPTIONAL
                                          iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                                CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_customer IMPORTING is_booking         TYPE /dmo/booking05
                                      is_bookingx        TYPE /dmo/if_flight_legacy05=>ts_bookingx OPTIONAL
                                      iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                            CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_flight IMPORTING is_booking         TYPE /dmo/booking05
                                    is_bookingx        TYPE /dmo/if_flight_legacy05=>ts_bookingx OPTIONAL
                                    iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                          CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_booking         TYPE /dmo/booking05
                                           is_bookingx        TYPE /dmo/if_flight_legacy05=>ts_bookingx OPTIONAL
                                           iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                                 CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _determine IMPORTING iv_change_mode TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                       CHANGING  cs_booking     TYPE /dmo/booking05.
ENDCLASS.
...
CLASS lcl_booking_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    SELECT FROM /dmo/booking05 FIELDS travel_id, booking_id FOR ALL ENTRIES IN @it_booking WHERE travel_id = @it_booking-travel_id AND booking_id = @it_booking-booking_id INTO CORRESPONDING FIELDS OF TABLE @lt_travel_booking_id.
    ...
    LOOP AT it_booking INTO DATA(ls_booking_create) ##INTO_OK.
      ...
      IF ls_booking_create-booking_id IS INITIAL.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_no_key  travel_id = ls_booking_create-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      IF sy-subrc = 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_exists  travel_id = ls_booking_create-travel_id  booking_id = ls_booking_create-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
  ...
  METHOD _update.
    ...
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking_update>) WHERE booking_id = 0.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_no_key  travel_id = <s_booking_update>-travel_id ) TO et_messages.
      RETURN.
    ENDLOOP.

    DATA lt_booking TYPE SORTED TABLE OF /dmo/booking05 WITH UNIQUE KEY travel_id  booking_id.
    SELECT * FROM /dmo/booking05 FOR ALL ENTRIES IN @it_booking WHERE travel_id = @it_booking-travel_id AND booking_id = @it_booking-booking_id INTO TABLE @lt_booking.

    FIELD-SYMBOLS <s_buffer_booking> TYPE /dmo/booking05.
    DATA ls_buffer_booking TYPE /dmo/booking05.
    LOOP AT it_booking ASSIGNING <s_booking_update>.
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_unknown  travel_id = <s_booking_update>-travel_id  booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      READ TABLE it_bookingx ASSIGNING FIELD-SYMBOL(<s_bookingx>) WITH KEY travel_id   = <s_booking_update>-travel_id
                                                                           ...
                                                                           action_code = /dmo/if_flight_legacy05=>action_code-update.
      IF sy-subrc <> 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>booking_no_control
      ...
      IF   ( <s_bookingx>-carrier_id    = abap_true AND ( <s_booking_update>-carrier_id    <> <s_buffer_booking>-carrier_id    ) )
        ...
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>booking_flight_u
                                          travel_id  = <s_booking_update>-travel_id
                                          booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      IF   ( <s_bookingx>-flight_price = abap_true  AND <s_bookingx>-currency_code = abap_false )
        ...
        " Price and currency code must be changed together
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>booking_price_currency_u
                                          travel_id  = <s_booking_update>-travel_id
                                          booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
  ...
  METHOD _delete.
    ...
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking_delete>) WHERE booking_id = 0.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_no_key  travel_id = <s_booking_delete>-travel_id ) TO et_messages.
      RETURN.
    ENDLOOP.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_booking_db TYPE /dmo/if_flight_legacy05=>tt_booking_key.
      SELECT travel_id, booking_id FROM /dmo/booking05 FOR ALL ENTRIES IN @lt_booking WHERE travel_id = @lt_booking-travel_id AND booking_id = @lt_booking-booking_id INTO CORRESPONDING FIELDS OF TABLE @lt_booking_db.
    ENDIF.
    ...
    LOOP AT lt_booking ASSIGNING <s_booking_delete>.
      IF iv_no_delete_check = abap_false.
        ...
        IF sy-subrc <> 0.
          APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_unknown  travel_id = <s_booking_delete>-travel_id  booking_id = <s_booking_delete>-booking_id ) TO et_messages.
          RETURN.
        ENDIF.
      ENDIF.
      INSERT VALUE #( travel_id = <s_booking_delete>-travel_id  booking_id = <s_booking_delete>-booking_id ) INTO TABLE mt_delete_buffer_2.
  ...
  METHOD save.
    ...
    ASSERT mt_update_buffer_2 IS INITIAL.
    ASSERT mt_delete_buffer_2 IS INITIAL.
    INSERT /dmo/booking05 FROM TABLE @mt_create_buffer.
    UPDATE /dmo/booking05 FROM TABLE @mt_update_buffer.
    DELETE /dmo/booking05 FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD check_booking_id." Here we can safely assume that the Travel ID has already been checked!
    ...
    IF iv_booking_id IS INITIAL.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_no_key  travel_id = iv_travel_id ) TO ct_messages.
      RETURN.
    ENDIF.

    IF line_exists( mt_delete_buffer[ travel_id = iv_travel_id  booking_id = iv_booking_id ] ).
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_unknown  travel_id = iv_travel_id  booking_id = iv_booking_id ) TO ct_messages.
      RETURN.
    ENDIF.
    ...
    SELECT SINGLE FROM /dmo/booking05 FIELDS @abap_true WHERE travel_id = @iv_travel_id AND booking_id = @iv_booking_id INTO @DATA(lv_db_exists).
  ...
  METHOD cud_prep.
    ...
    DATA lt_booking_c  TYPE /dmo/if_flight_legacy05=>tt_booking.
    DATA lt_booking_u  TYPE /dmo/if_flight_legacy05=>tt_booking.
    DATA lt_booking_d  TYPE /dmo/if_flight_legacy05=>tt_booking.
    DATA lt_bookingx_u TYPE /dmo/if_flight_legacy05=>tt_bookingx.
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking>).
      ...
      IF sy-subrc <> 0.
        ...
        RETURN.
      ENDIF.
      CASE CONV /dmo/if_flight_legacy05=>action_code_enum( <s_bookingx>-action_code ).
        WHEN /dmo/if_flight_legacy05=>action_code-create.
  ...
  METHOD get.
    ...
    SELECT * FROM /dmo/booking05 FOR ALL ENTRIES IN @it_booking WHERE travel_id  = @it_booking-travel_id
  ...
  METHOD _check_booking_date.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_bookingx-booking_date = abap_true ).

      " A. Booking Date must not be initial
      " B. When the record is created it must not be in the past
      IF is_booking-booking_date IS INITIAL OR is_booking-booking_date = '' OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create AND is_booking-booking_date < cl_abap_context_info=>get_system_date( ) ).
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>booking_booking_date_invalid  travel_id = is_booking-travel_id  booking_id = is_booking-booking_id  booking_date = is_booking-booking_date ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_bookingx-customer_id = abap_true ).
  ...
  METHOD _check_flight.
    rv_is_valid = abap_true.
    IF     iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create
      OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update
      ...
      IF mt_flight_key IS INITIAL.
        SELECT carrier_id, connection_id, flight_date FROM /dmo/flight05 INTO CORRESPONDING FIELDS OF TABLE @mt_flight_key.
      ENDIF.
      ...
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>flight_unknown  carrier_id = is_booking-carrier_id  connection_id = is_booking-connection_id  flight_date = is_booking-flight_date ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create AND is_booking-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_bookingx-currency_code = abap_true ).
  ...
  METHOD _determine.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create.
      ...
      IF cs_booking-flight_price IS INITIAL OR cs_booking-currency_code IS INITIAL.
        ...
        SELECT SINGLE price, currency_code FROM /dmo/flight05 WHERE carrier_id    = @cs_booking-carrier_id
...
CLASS lcl_travel_buffer DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    ...
    METHODS set_status_to_booked IMPORTING iv_travel_id TYPE /dmo/travel_id05
                                 EXPORTING et_messages  TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS save.
    METHODS initialize.
    METHODS check_travel_id IMPORTING iv_travel_id       TYPE /dmo/travel_id05
                            CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    "! Prepare changes in a temporary buffer
    "! @parameter iv_no_delete_check | In some cases we do not need to check the existence of a record to be deleted, as this check has been done before.
    "!                               | E.g. delete all subnodes of a node to be deleted.  In this case we have read the subnodes to get their keys.
    METHODS cud_prep IMPORTING it_travel          TYPE /dmo/if_flight_legacy05=>tt_travel
                               it_travelx         TYPE /dmo/if_flight_legacy05=>tt_travelx
                               iv_no_delete_check TYPE abap_bool OPTIONAL
                     EXPORTING et_travel          TYPE /dmo/if_flight_legacy05=>tt_travel
                               et_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    "! Add content of the temporary buffer to the real buffer and clear the temporary buffer
    METHODS cud_copy.
    "! Discard content of the temporary buffer
    METHODS cud_disc.
    METHODS get IMPORTING it_travel              TYPE /dmo/if_flight_legacy05=>tt_travel
                          iv_include_buffer      TYPE abap_boolean
                          iv_include_temp_buffer TYPE abap_boolean
                EXPORTING et_travel              TYPE /dmo/if_flight_legacy05=>tt_travel.
  ...
  PRIVATE SECTION.
    ...
    " Main buffer
    DATA: mt_create_buffer TYPE /dmo/if_flight_legacy05=>tt_travel,
          mt_update_buffer TYPE /dmo/if_flight_legacy05=>tt_travel,
          mt_delete_buffer TYPE /dmo/if_flight_legacy05=>tt_travel_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_travel,
          mt_update_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_travel,
          mt_delete_buffer_2 TYPE /dmo/if_flight_legacy05=>tt_travel_key.

    DATA mt_agency_id   TYPE SORTED TABLE OF /dmo/agency_id05   WITH UNIQUE KEY table_line.

    METHODS _create IMPORTING it_travel   TYPE /dmo/if_flight_legacy05=>tt_travel
                    EXPORTING et_travel   TYPE /dmo/if_flight_legacy05=>tt_travel
                              et_messages TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _update IMPORTING it_travel   TYPE /dmo/if_flight_legacy05=>tt_travel
                              it_travelx  TYPE /dmo/if_flight_legacy05=>tt_travelx
                    EXPORTING et_travel   TYPE /dmo/if_flight_legacy05=>tt_travel
                              et_messages TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.
    METHODS _delete IMPORTING it_travel          TYPE /dmo/if_flight_legacy05=>tt_travel
                              iv_no_delete_check TYPE abap_bool
                    EXPORTING et_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message.

    METHODS _check IMPORTING is_travel          TYPE /dmo/travel05
                             is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                             iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                   CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_agency IMPORTING is_travel          TYPE /dmo/travel05
                                    is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                                    iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                          CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_customer IMPORTING is_travel          TYPE /dmo/travel05
                                      is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                                      iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                            CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_dates IMPORTING is_travel          TYPE /dmo/travel05
                                   is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                                   iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                         CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                         RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_status IMPORTING is_travel          TYPE /dmo/travel05
                                    is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                                    iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                          CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_travel          TYPE /dmo/travel05
                                           is_travelx         TYPE /dmo/if_flight_legacy05=>ts_travelx OPTIONAL
                                           iv_change_mode     TYPE /dmo/cl_flight_legacy05=>ty_change_mode
                                 CHANGING  ct_messages        TYPE /dmo/if_flight_legacy05=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _update_admin IMPORTING iv_new TYPE abap_bool CHANGING cs_travel_admin TYPE /dmo/travel_a_05.
ENDCLASS.
...
CLASS lcl_travel_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    DATA lv_travel_id_max TYPE /dmo/travel_id05.
    IF lcl_travel_buffer=>get_instance( )->mt_create_buffer IS INITIAL.
      SELECT FROM /dmo/travel05 FIELDS MAX( travel_id ) INTO @lv_travel_id_max.
    ELSE.
    ...
    LOOP AT it_travel INTO DATA(ls_travel_create) ##INTO_OK.
      ...
      ls_travel_create-status = /dmo/if_flight_legacy05=>travel_status-new.
  ...
  METHOD _update.
    ...
    IF sy-subrc = 0.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_no_key ) TO et_messages.
      RETURN.
    ENDIF.

    DATA lt_travel TYPE SORTED TABLE OF /dmo/travel05 WITH UNIQUE KEY travel_id.
    SELECT * FROM /dmo/travel05 FOR ALL ENTRIES IN @it_travel WHERE travel_id = @it_travel-travel_id INTO TABLE @lt_travel ##SELECT_FAE_WITH_LOB[DESCRIPTION].

    FIELD-SYMBOLS <s_buffer_travel> TYPE /dmo/travel05.
    DATA ls_buffer_travel TYPE /dmo/travel05.
    LOOP AT it_travel ASSIGNING FIELD-SYMBOL(<s_travel_update>).
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_unknown  travel_id = <s_travel_update>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      " Merge fields to be updated
      READ TABLE it_travelx ASSIGNING FIELD-SYMBOL(<s_travelx>) WITH KEY travel_id = <s_travel_update>-travel_id  action_code = /dmo/if_flight_legacy05=>action_code-update.
      IF sy-subrc <> 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_no_control  travel_id = <s_travel_update>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      DATA lv_field TYPE i.
      lv_field = 3.
  ...
  METHOD _delete.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_travel_db TYPE SORTED TABLE OF /dmo/travel_id05 WITH UNIQUE KEY table_line.
      SELECT travel_id FROM /dmo/travel05 FOR ALL ENTRIES IN @lt_travel WHERE travel_id = @lt_travel-travel_id INTO TABLE @lt_travel_db.
    ENDIF.
    ...
    LOOP AT lt_travel ASSIGNING <s_travel_delete>.
      IF iv_no_delete_check = abap_false.
        ...
        IF sy-subrc <> 0.
          APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_unknown  travel_id = <s_travel_delete>-travel_id ) TO et_messages.
          RETURN.
        ENDIF.
      ENDIF.
      INSERT VALUE #( travel_id = <s_travel_delete>-travel_id ) INTO TABLE mt_delete_buffer_2.
  ...
  METHOD set_status_to_booked.
    ...
    IF sy-subrc = 0." Error: Record of action marked for deletion
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_unknown  travel_id = iv_travel_id ) TO et_messages.
      RETURN.
    ENDIF.
    ...
    IF <s_travel> IS NOT ASSIGNED.
      SELECT SINGLE * FROM /dmo/travel05 WHERE travel_id = @iv_travel_id INTO @DATA(ls_travel) .
      IF sy-subrc = 0.
    ...
    <s_travel>-status = /dmo/if_flight_legacy05=>travel_status-booked.
  ...
  METHOD save.
    ...
    ASSERT mt_update_buffer_2 IS INITIAL.
    ASSERT mt_delete_buffer_2 IS INITIAL.
    INSERT /dmo/travel05 FROM TABLE @mt_create_buffer.
    UPDATE /dmo/travel05 FROM TABLE @mt_update_buffer.
    DELETE /dmo/travel05 FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD check_travel_id.
    ...
    IF iv_travel_id IS INITIAL.
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_no_key ) TO ct_messages.
      RETURN.
    ENDIF.

    IF line_exists( mt_delete_buffer[ travel_id = iv_travel_id ] ).
      APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_unknown  travel_id = iv_travel_id ) TO ct_messages.
      RETURN.
    ENDIF.
    ...
    SELECT SINGLE FROM /dmo/travel05 FIELDS @abap_true WHERE travel_id = @iv_travel_id INTO @DATA(lv_db_exists).
    IF lv_db_exists = abap_true.
      rv_is_valid = abap_true.
      RETURN.
    ENDIF.
  ...
  METHOD cud_prep.
    ...
    DATA lt_travel_c  TYPE /dmo/if_flight_legacy05=>tt_travel.
    DATA lt_travel_u  TYPE /dmo/if_flight_legacy05=>tt_travel.
    DATA lt_travel_d  TYPE /dmo/if_flight_legacy05=>tt_travel.
    DATA lt_travelx_u TYPE /dmo/if_flight_legacy05=>tt_travelx.
    LOOP AT it_travel ASSIGNING FIELD-SYMBOL(<s_travel>).
      ...
      IF sy-subrc <> 0.
        APPEND NEW /dmo/cx_flight_legacy05( textid     = /dmo/cx_flight_legacy05=>travel_no_control
                                          travel_id  = <s_travel>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      CASE CONV /dmo/if_flight_legacy05=>action_code_enum( <s_travelx>-action_code ).
        WHEN /dmo/if_flight_legacy05=>action_code-create.
  ...
  METHOD get.
    ...
    SELECT * FROM /dmo/travel05 FOR ALL ENTRIES IN @it_travel WHERE travel_id = @it_travel-travel_id INTO TABLE @et_travel ##SELECT_FAE_WITH_LOB[DESCRIPTION].
  ...
  METHOD _check_agency.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-agency_id = abap_true ).
      IF mt_agency_id IS INITIAL.
        SELECT DISTINCT agency_id FROM /dmo/agency05 INTO TABLE @mt_agency_id.
      ENDIF.
      READ TABLE mt_agency_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = is_travel-agency_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>agency_unkown  agency_id = is_travel-agency_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-customer_id = abap_true ).
  ...
  METHOD _check_dates.
    ...
    " begin date
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-begin_date = abap_true ).
      IF is_travel-begin_date IS INITIAL OR is_travel-begin_date = ''.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>no_begin_date  travel_id = is_travel-travel_id ) TO ct_messages.
      ENDIF.
    ENDIF.

    " end date
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-end_date = abap_true ).
      IF is_travel-end_date IS INITIAL OR is_travel-end_date = ''.
        rv_is_valid = abap_false.
        APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>no_end_date  travel_id = is_travel-travel_id ) TO ct_messages.
      ENDIF.
    ENDIF.

    " begin date < = end date
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-begin_date = abap_true )
                                                                  OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-end_date   = abap_true ).
      IF is_travel-begin_date IS NOT INITIAL AND is_travel-end_date IS NOT INITIAL.
        IF is_travel-begin_date > is_travel-end_date.
          rv_is_valid = abap_false.
          APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>end_date_before_begin_date  begin_date = is_travel-begin_date  end_date = is_travel-end_date  travel_id = is_travel-travel_id ) TO ct_messages.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_status.
    rv_is_valid = abap_true.
    IF iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-status = abap_true.
      CASE is_travel-status.
        WHEN CONV /dmo/travel_status05( /dmo/if_flight_legacy05=>travel_status-booked ).   " OK
        WHEN CONV /dmo/travel_status05( /dmo/if_flight_legacy05=>travel_status-cancelled )." OK
        WHEN CONV /dmo/travel_status05( /dmo/if_flight_legacy05=>travel_status-new ).      " OK
        WHEN CONV /dmo/travel_status05( /dmo/if_flight_legacy05=>travel_status-planned ).  " OK
        WHEN OTHERS.
          rv_is_valid = abap_false.
          APPEND NEW /dmo/cx_flight_legacy05( textid = /dmo/cx_flight_legacy05=>travel_status_invalid  status = is_travel-status ) TO ct_messages.
      ENDCASE.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    ...
    ENDIF.
    IF   iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-create
      OR ( iv_change_mode = /dmo/cl_flight_legacy05=>change_mode-update AND is_travelx-currency_code = abap_true ).
