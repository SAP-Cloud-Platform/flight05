...
CLASS lcl_agency_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_agency TYPE STANDARD TABLE OF /dmo/agency05 WITH KEY agency_id.
...
CLASS lcl_airport_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_airport TYPE STANDARD TABLE OF /dmo/airport05 WITH KEY airport_id.
    CLASS-METHODS: get_data
...
CLASS lcl_airport_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/airport05.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data(  ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/airport05 FROM TABLE @lt_data.
...
CLASS lcl_carrier_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_carrier TYPE STANDARD TABLE OF /dmo/carrier05 WITH KEY carrier_id.
    CLASS-METHODS: get_data
      RETURNING
...
CLASS lcl_carrier_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/carrier05.                           "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/carrier05 FROM TABLE @lt_data.
...
CLASS lcl_connection_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_connection TYPE STANDARD TABLE OF /dmo/connecti_05 WITH KEY carrier_id connection_id.
...
CLASS lcl_connection_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ). ENDIF.
    DELETE FROM /dmo/connecti_05.                        "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT /dmo/connecti_05 FROM TABLE @lt_data.
...
CLASS /dmo/cl_flight_data_generat_05 DEFINITION LOCAL FRIENDS lcl_flight_data_generator.
CLASS lcl_flight_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_flights TYPE STANDARD TABLE OF /dmo/flight05
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF ty_plane_type,
             id            TYPE /dmo/plane_type_id05,
             seats_max     TYPE /dmo/plane_seats_max05,
           ...
           BEGIN OF ty_flight_info,
             ...
             long_distance  TYPE abap_bool,
             seats_max      TYPE /dmo/plane_seats_max05,
             seats_occupied TYPE /dmo/plane_seats_occupied05,
             price          TYPE /dmo/flight_price05,
           END OF ty_flight_info,
           BEGIN OF ty_connection_recurrency,
             id         TYPE /dmo/connection_id05,
             recurrency TYPE STANDARD TABLE OF /dmo/flight_date05 WITH EMPTY KEY,
    ...
    CLASS-METHODS: build_dependent_content,
      get_flight_info
        IMPORTING
          iv_connection_id     TYPE /dmo/connection_id05
        RETURNING
...
CLASS lcl_flight_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/flight05.                            "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ).  ENDIF.
    INSERT /dmo/flight05 FROM TABLE @gt_flights.
    ...
    LOOP AT gt_connections INTO DATA(ls_connection).
      LOOP AT gt_connection_recurrency[ id = ls_connection-connection_id ]-recurrency INTO DATA(lv_flight_date).
        ...
        APPEND VALUE /dmo/flight05(
                  ...
                  currency_code  = VALUE /dmo/flight05-currency_code(
  ...
  METHOD get_flight_info.
    ...
    rs_plane_info = VALUE ty_flight_info(
        ...
        price          = /dmo/cl_flight_data_generat_05=>calculate_flight_price(
  ...
  METHOD build_connection_recurrency.
    ...
    flight_date_max = calc_next_monday( CONV /dmo/flight_date05( lv_datum + 217 ) ).

    " flight_date_min is a Monday 5 months in the past
    flight_date_min = calc_next_monday( CONV /dmo/flight_date05( lv_datum - 148 ) ).
      ...
      APPEND VALUE ty_connection_recurrency(
          ...
          recurrency = VALUE ty_connection_recurrency-recurrency(
                                    ...
                                    UNTIL flightdate < flight_date_min + ls_connection-weekday - 1
                                       ( CONV /dmo/flight_date05( flightdate ) )
                               )
      ) TO rt_data.
    ENDLOOP.
  ENDMETHOD.
...
CLASS lcl_customer_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_last_name,
             last_name TYPE /dmo/last_name05,
           END OF ty_last_name.
    TYPES: tt_customer  TYPE STANDARD TABLE OF /dmo/customer05 WITH KEY customer_id,
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      BEGIN OF ty_first_name,
        first_name TYPE /dmo/first_name05,
        gender     TYPE c LENGTH 1,
      END OF ty_first_name,
      BEGIN OF ty_name,
        first_name TYPE /dmo/first_name05,
        last_name  TYPE /dmo/last_name05,
        title      TYPE /dmo/title05,
      END OF ty_name,
      " Addresses
      BEGIN OF ty_city,
        country     TYPE land1,
        postal_code TYPE /dmo/postal_code05,
        city        TYPE /dmo/city05,
      END OF ty_city,
      tt_street_per_country TYPE STANDARD TABLE OF /dmo/street05 WITH EMPTY KEY,
      BEGIN OF ty_street,
        country TYPE land1,
      ...
      BEGIN OF ty_address,
        ...
        postal_code   TYPE /dmo/postal_code05,
        city          TYPE /dmo/city05,
        street        TYPE /dmo/street05,
        phone_number  TYPE  /dmo/phone_number05,
        email_address TYPE /dmo/email_address05,
      END OF ty_address.
...
CLASS lcl_customer_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/customer05.                          "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/customer05 FROM TABLE @lt_data.
    ...
    LOOP AT build_last_names( ) INTO DATA(ls_last_name).
      DO lo_random_counter->get_next( ) TIMES.
        ...
        APPEND VALUE ty_name(
            ...
            title      = COND /dmo/title05( WHEN ls_first_name-gender = 'M' THEN 'Mr.'
...
CLASS lcl_supplement_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_supplement      TYPE STANDARD TABLE OF /dmo/suppleme_05 WITH KEY supplement_id,
           tt_supplement_text TYPE STANDARD TABLE OF /dmo/suppl_te_05 WITH KEY supplement_id.

    " Merged types
    TYPES BEGIN OF ty_supplement_complete.
    INCLUDE TYPE /dmo/suppleme_05.
    TYPES language_code TYPE spras.
    TYPES description   TYPE /dmo/description05.
...
CLASS lcl_supplement_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ).  ENDIF.
    DELETE FROM /dmo/suppleme_05.                        "#EC CI_NOWHERE
    DELETE FROM /dmo/suppl_te_05.                        "#EC CI_NOWHERE

    IF out IS BOUND.  out->write( '--> Build Content.' ). ENDIF.
    DATA(lt_data) = get_data( ).

    IF out IS BOUND.  out->write( '--> Insert Content.' ). ENDIF.
    INSERT /dmo/suppleme_05 FROM TABLE @( CORRESPONDING tt_supplement( lt_data ) ).
    INSERT /dmo/suppl_te_05 FROM TABLE @( CORRESPONDING tt_supplement_text( lt_data ) ).
...
CLASS /dmo/cl_flight_data_generat_05 DEFINITION LOCAL FRIENDS lcl_travel_data_generator.
CLASS lcl_travel_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.

    TYPES: tt_travel              TYPE STANDARD TABLE OF /dmo/travel05     WITH KEY travel_id,
           tt_bookings            TYPE STANDARD TABLE OF /dmo/booking05    WITH KEY travel_id booking_id,
           tt_booking_supplements TYPE STANDARD TABLE OF /dmo/book_sup_05 WITH KEY travel_id booking_id booking_supplement_id.

    " Build nested tables
    TYPES BEGIN OF ty_booking_complete.
    INCLUDE TYPE /dmo/booking05.
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      ...
      build_booking
        IMPORTING
          iv_travel_id       TYPE /dmo/booking05-travel_id
        RETURNING
      ...
      find_next_fitting_flight
        IMPORTING
          ...
          is_flight_previous TYPE /dmo/flight05 OPTIONAL
        RETURNING
          VALUE(rs_flight)   TYPE /dmo/flight05,
      generate_booking_supplements
        IMPORTING
          iv_travel_id   TYPE /dmo/booking05-travel_id
          iv_booking_id  TYPE /dmo/booking05-booking_id
        RETURNING
      ...
      generate_description
        ...
        RETURNING
          VALUE(rv_description) TYPE /dmo/travel05-description,
      ...
      calculate_booking_fee
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_booking_fee) TYPE /dmo/travel05-booking_fee,
      generate_travel_customer_id
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_customer_id) TYPE /dmo/travel05-customer_id,
      set_today,
...
CLASS lcl_travel_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    LOOP AT lt_data INTO DATA(ls_travel).
      APPEND CORRESPONDING /dmo/travel05( ls_travel ) TO lt_travels.
      LOOP AT ls_travel-bookings INTO DATA(ls_booking).
        APPEND CORRESPONDING /dmo/booking05( ls_booking ) TO lt_bookings.
    ...
    INSERT /dmo/travel05 FROM TABLE @lt_travels.
    IF out IS BOUND.  out->write( '--> Insert Booking Content' ).  ENDIF.
    INSERT /dmo/booking05 FROM TABLE @lt_bookings.
    IF out IS BOUND.  out->write( '--> Insert Booking Supplement Content' ).  ENDIF.
    INSERT /dmo/book_sup_05 FROM TABLE @lt_booking_supplements.
  ...
  METHOD get_data.
    DATA: lv_travel_id TYPE /dmo/booking05-travel_id.
    ...
    WHILE lt_bookings IS NOT INITIAL.
      ...
      APPEND VALUE ty_travel_complete(
          ...
          total_price   = lv_booking_fee + REDUCE /dmo/travel05-total_price( INIT sum = 0
                                                          FOR booking IN lt_bookings
                                                          NEXT
                                                           sum = sum
                                                                 + booking-flight_price
                                                                 + REDUCE /dmo/flight_price05( INIT sum_supplement = 0
          ...
          status        = SWITCH /dmo/travel05-status( go_ran_status_description->get_next( )
                                          WHEN 1 OR 2 THEN /dmo/if_flight_legacy05=>travel_status-new
                                          WHEN 3      THEN /dmo/if_flight_legacy05=>travel_status-booked
                                          WHEN 4      THEN /dmo/if_flight_legacy05=>travel_status-planned  )
  ...
  METHOD build_booking.
    TYPES: tt_customer_id TYPE TABLE OF /dmo/customer05-customer_id WITH EMPTY KEY.
    DATA: lv_booking_id      TYPE /dmo/booking05-booking_id,
    ...
    DO lv_trip_length TIMES.
      ...
      DATA(lv_booking_date) = CONV /dmo/booking05-booking_date( <flight>-flight_date - go_ran_booking_date->get_next( ) ).

      DATA(lv_price) = /dmo/cl_flight_data_generat_05=>calculate_flight_price(
      ...
      FOR i = 1  THEN i + 1  WHILE i <= lines( lt_customer_id )
         (
            ...
            booking_supplements = generate_booking_supplements( iv_travel_id = iv_travel_id   iv_booking_id = CONV /dmo/booking05-booking_id( lv_booking_id + i ) )
          )
      ) TO rt_bookings.
  ...
  METHOD find_next_fitting_flight.
    ...
    IF is_flight_previous IS SUPPLIED.
      DATA(lv_connection_id_new) = VALUE /dmo/connecti_05-connection_id( gt_connections[
  ...
  METHOD generate_description.
    TYPES: tt_customers TYPE SORTED TABLE OF /dmo/customer05 WITH UNIQUE KEY customer_id.
    rv_description = SWITCH /dmo/travel05-description(
                               go_ran_travel_description->get_next( )
                                  WHEN 1 THEN `Business Trip for ` &&
                                                REDUCE /dmo/travel05-description(
                                                               ...
                                                               NEXT s =  s && gt_customer[ customer_id = traveler-customer_id ]-first_name
                                                                           && COND /dmo/travel05-description( WHEN i < lines( it_bookings ) THEN `, ` )
                                                                    i = i + 1 )
  ...
  METHOD generate_travel_customer_id.
    rv_customer_id = COND /dmo/travel05-customer_id(
